...
namespace Loja
{
    public partial class F_TelaMovimentacao : Form
    {
        ...
        private void Movimentacao()
        {
            ...
            try
            {
                var sql = "SELECT estoque_minimo, estoque_atual FROM produto WHERE id = @id";
                ...
                using (var command = new MySqlCommand(sql, connection))
                {
                    ...
                    quantidadeMinima = reader["estoque_minimo"].ToString();
                    estoqueAtual = reader["estoque_atual"].ToString();
        ...
        private void Atualizar(int produtoId, int userId, int estoque, string tipo, int quantidade)
        {
            using (MySqlConnection conexao = new MySqlConnection(stringConnection))
            {
                ...
                try
                {
                    ...
                    string cmdUpdate = $"UPDATE produto SET estoque_atual = @estoque WHERE id = @id";
                    string cmdInsert = "INSERT INTO movimentacao (id_usuario, id_produto, tipo, quantidade) VALUES (@id_usuario,@id_produto, @tipo, @quantidade)";
                    ...
                    comando2.Parameters.AddWithValue("@id_produto", produtoId);
